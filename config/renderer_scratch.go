...
func (s *Scratch) MapGet(k, mk string) (interface{}, error) {
	...
	v, ok := s.values[k]
	if !ok {
		return nil, fmt.Errorf("Key %s do not exist in scratch", k)
	}

	// TODO: why is this a []map ?
	v2, ok := v.([]map[string]interface{})
	if !ok {
		return nil, fmt.Errorf("Map %s is not of type map[string]interface{}", k)
	}

	v3, ok := v2[0][mk]
	if !ok {
		return nil, fmt.Errorf("Map key %s do not exist in scratch[%s]", mk, k)
	}
	return v3, nil
}
